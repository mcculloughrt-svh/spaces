...
export async function listProjects(options: {
  ...
  for (const project of projects) {
    ...
    if (options.verbose) {
      logger.log(
        `${indicator} ${project.name.padEnd(20)} ${project.repository.padEnd(30)} ${project.workspaceCount} workspaces${currentLabel}`
      );
    } else {
      logger.log(
        `${indicator} ${project.name.padEnd(20)} ${project.repository}${currentLabel}`
      );
    }
  }
}
...
export async function listWorkspaces(options: {
  ...
  for (const workspace of workspaces) {
    ...
    if (workspace.ahead > 0 || workspace.behind > 0) {
      parts.push(`${workspace.branch} +${workspace.ahead} -${workspace.behind}`.padEnd(20));
    } else {
      parts.push(`${workspace.branch}`.padEnd(20));
    }
