...
export async function removeWorkspace(
  ...
  if (workspaceNameArg) {
    ...
    // Select workspace
    const selected = await selectItem(workspaceNames, 'Select workspace to remove:');
  ...
  if (!options.force) {
    const confirmed = await promptConfirm(`Remove workspace "${workspaceName}"?`, false);
  ...
  await runScriptsInTerminal(removeScriptsDir, workspacePath, workspaceName, projectConfig.repository);
  ...
  if (!options.keepBranch) {
    ...
    if (deleteBranch) {
      try {
        ...
        logger.warning(`Could not delete branch: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  }
}
...
export async function removeProject(
  ...
  if (projectNameArg) {
    ...
    const selected = await selectItem(projectOptions, 'Select project to remove:');
  ...
  if (!options.force) {
    const confirmName = await promptInput(`Type the project name "${projectName}" to confirm:`);
